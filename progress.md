...
## Why our exp04 number was 0/200 — four defects in the policy path

**Symptom.** exp04 scored `demo_success 0/200` here while zjw's own eval scored
**79/200** on the same checkpoint, the same 200 held-out cases, and the same 647
frames. Per case, same `episode_uid`: he lifted the cube to z=1.059 m, ours moved
it **5.8 micrometres**.

**Not the policy, and not the physics.** Teacher replay on the same 0821 data
passed 8/8 at guide 4.8 mm — better than the 0818 baseline. Four separate defects
in the *policy* path, found in this order, each one hiding the next:

**A. One runner shared across N parallel envs.** `bench_replay.py:401`
(`if policy_mode and index > 0: continue`) resets only for case 0, so a shard's N
cases ran as N parallel envs sharing one runner and **one plan** — cases 1..N-1
chased case 0's target. In the 200-case run each worker held 50 cases.
*Diagnostic:* one `plan received` in the server log while `ticks = frames × n`.
*Effect on case 0:* guide 158.5 mm → **20.8 mm** when run one-env, matching zjw's
19.4 mm. The README already warned: one model instance per env, a shared one
contaminates them all. `policy_server_flow.py` shows the per-`env_id` pattern.

**B. Plan cropped from the wrong source offset.** zjw slices
`policy_plan[121:768]`; we sliced `[0:647]`. Same length, **content shifted by 121
frames**. The object only starts moving near frame 463, so our plan told the
policy to wait, and it then chased and diverged.
*Diagnostic — the sharpest one:* the hand was nearly perfect while the arm was
not. Action MAE over the first 300 frames: hand **0.001 rad** (zjw 0.0096), arm
**0.384 rad** (zjw 0.057, a 6.7× gap). The hand is *absolute* so plan timing does
not affect it; the arm tracks the plan, so the whole error lands there. Per-frame,
arm j7: ours 0.0119 → 0.598 → **1.250** rad at frames 0/50/100 while the teacher
has not moved at all. Frame 0 is nearly exact, so this is not a wrong starting
pose — it is a wrong plan.
**`start_frame` and the plan crop are two independent offsets.** `suite.start_frame`
only chooses where the sim begins; the plan was still cut from `[:horizon]`. That
is why setting `start_frame=121` alone changed nothing (20.8 mm, bit-identical).
Suite now has `plan_source_start`.

**C. The chunk cache was dropped every tick.** Three things stacked:
`SubprocessAdapter.commit_action()` inherited the empty base method so nothing
crossed the wire; `chunk_invalidation_tolerance_rad=0.0` compares against the
*previous frame's* action, which always differs on a moving robot; and the commit
happened *before* the clamp, so the committed value was not what PhysX executed.

**D. The recorded initial joint position never crossed the wire.**
`SubprocessAdapter.reset()` accepted `robot_joint_pos` and dropped it, so a server
could only use the joints it read on its first tick — the post-settle simulator
readback. zjw seeds from `episode.arrays["robot_joint_pos"][start]`. For a
delta-action policy that is the base every delta accumulates from. Fixed with a
new `INIT_STATE` frame. (Measured effect on its own: none — but it was wrong.)

Fixing C alone left the score at 0/4, and D alone changed nothing. That is what
pointed at A and B. **Do not stop at the first bug you find.**

**The diagnostic that proves it, and the one to check first from now on:**

```
inference_calls / frames  ==  647/647  ==  1.00     <- broken, replans every tick
                              ~21/647  ==  0.05     <- healthy, chunk cache works
```

A chunk that is thrown away every tick never runs to completion, so a chunked
grasp never finishes. The server now reports `calls_per_tick` in its stats line
for exactly this reason.

**Why replay passing did not catch it.** Replay uses the `recorded_teacher`
adapter, which bypasses the commit/chunk code entirely. So "replay passes,
therefore the platform is fine" is valid for physics, loader, and profile — but
**not** for the policy path. `docs/ARCHITECTURE.md` said as much under "Not done
yet": `--policy-cmd` had never carried a real checkpoint end to end.

**Why the CPU smoke test did not catch it either.** It fed near-static
observations, so consecutive actions barely differed and the zero tolerance never
tripped. A protocol smoke test proves framing, not semantics. Check
`calls_per_tick`, not just "actions came back finite".

**The fix**, three layers, each with a `.bak_*` beside it:

| file | change |
|---|---|
| `interprior_bench/protocol.py` | new `COMMIT = -6.0` and `INIT_STATE = -7.0` frames |
| `adapters/subprocess_adapter.py` | implement `commit_action`; send the recorded initial joints in `reset()` |
| `suites/suite.py` | new `plan_source_start` field (defect B) |
| `scripts/bench_replay.py` | crop the plan from `plan_source_start`; commit *after* the clamp and commit the *clamped* value; new `--chunk-invalidation-tolerance-rad` (default 1e-3) |
| `train_eval/policy_server_exp04.py` | handle `COMMIT` + `INIT_STATE`; `--fixed-replan-schedule`; report `calls_per_tick` |

**Still open: defect A.** The per-env fix is not written yet — measurements above
used `--case-indices <one>` to sidestep it. Either build one runner per `env_id` in
the server, or have the driver run one case per process.

### Matching zjw's inference settings

His per-case `metrics.json` records the whole inference contract — read it rather
than guessing. What differed from our defaults:

| param | zjw | ours (was) |
|---|---|---|
| `max_target_step_rad` | **100.0** (effectively uncapped) | 0.05 |
| `chunk_invalidation_tolerance_rad` | **1e-3** | 0.0 |
| `fixed_replan_schedule` | on | absent |
| `policy_amp` | **True** | False |
| `start_frame` / `plan_frame_start` | **121** | 0 |

He still reports `safety_clipped_values=333` at a cap of 100.0, which says exp04's
raw actions are large — so a 0.05 cap silently throttles it. Note `api.py` does
**not** forward `--max-target-step-rad`; call `scripts/bench_replay.py` directly,
or add the passthrough.

`start_frame=121` alone does **not** fix the score (measured: still 0/4) because
frames 0–121 are bit-identical. Align it anyway for comparability.
...
**`--frames` overrides the suite's `max_frames`, silently.** Its default is `0`,
which means *full recording* (2400 frames) — not "use the suite's value". A suite
saying `max_frames: 647` still runs 2400 frames unless `--frames 647` is passed.
This bit the first exp04 run: guide tracking was averaged over 1753 frames of
post-plan drift, making the number pessimistic and not comparable to 768-frame
results. Pass `--frames` explicitly to match the plan horizon, or fix the
precedence so the suite wins.
