...
def _run(args: argparse.Namespace, root: Path) -> int:
    ...
    from bench_cube_val.envs.authored_episode import build_authored_episode
    ...
    #   derived_shard  a real recorded episode (the default, all cube/objaverse work)
    #   authored_npz   a hand-drawn object trajectory, with robot and scene
    #                  borrowed from a donor shard (envs/authored_episode.py)
    # The rollout, scoring and viewer code below is shared: an authored episode
    # satisfies the same DerivedEpisode contract, so nothing downstream branches.
    ...
    for case in cases:
        if authored:
            ...
            donor_slot = int(
                getattr(case, "donor_slot", None)
                if getattr(case, "donor_slot", None) is not None
                else getattr(suite, "donor_slot", 0)
            )
            episodes.append(
                build_authored_episode(
                    goal_npz, donor_shard, donor_slot, frames=args.frames
                )
            )
        else:
            episodes.append(
    ...
    for i in html_ids:
        ...
        # An authored trajectory IS its full length -- there is no shard behind it
        # to reload, and `episodes[i]` already holds every frame.
        recorded_full = np.asarray(
            (
                episodes[i]
                if authored
                else load_derived_episode(cases[i].shard, cases[i].slot, frames=0)
