...
def main():
    ...
    ap.add_argument("--force-robot-base", default="",
                    help="override the robot_base recorded IN the json (e.g. 0,0,0.53). "
                         "Use when re-expressing an old authored file in the real "
                         "simulator frame; without it the json's own value is kept.")
    ...
    ap.add_argument("--robot-base", default="0.0,0.0,0.53",
                    help="real env robot_root_pose_w. The old default 0.0,-0.08,0.53 "
                         "matched neither the authoring page nor the simulator.")
    ...
    if a.goal_json:
        ...
        # The json's own robot_base wins by default: it records the frame the
        # trajectory was actually drawn in, and silently re-basing would move the
        # path rather than reinterpret it. But the early authoring pages declared
        # [-0.6, 0, 0.6] while the simulator's robot sits at [0, 0, 0.53], so
        # those files need an explicit override to be re-expressed in the real
        # frame -- hence --force-robot-base.
        if a.force_robot_base:
            rb = [float(x) for x in a.force_robot_base.split(",")]
            print(f"FORCED robot_base {rb} (json declared {d.get('robot_base')})")
        else:
