#!/usr/bin/env python
"""Reach-trajectory viewer WITH the real robot + environment (like hammer_scene_real).

Extends build_reach_viewer.py: the reach dirs only carry an authored cube goal
trajectory (goal_traj[K,7], no robot motion). This adds the REAL xArm7+Wuji
robot meshes (URDF) at its home/rest pose and the DexVerse tabletop, so each
reach trajectory is shown in the robot's actual workspace for scale/context.

Robot pose source: frame 0 of a physics recording (hammer_playback.npz) which
holds the post-reset home pose of all 34 links in the DexVerse world frame.
Because the robot is STATIC here (one pose), each mesh world matrix
  T_world<-link(frame0) @ T_link<-mesh(static, from yourdfpy)
is precomputed once in Python and baked as a single 4x4 -- no per-frame robot
work in the browser. The cube animates along goal_traj as before.
"""
...
REACH_MAX = 0.907  # MEASURED: donor 3D max. Was 1.2 (user-set) -- see header.
REACH_SAFE = 0.687 # MEASURED: donor xy radius max
CUBE = 0.06        # real cube edge
...
def robot_static_meshes(pose_npz, frame=0):
    """Return {node: {v,i,c, m(16 world matrix at `frame`)}} using recorded link poses."""
    z = np.load(pose_npz, allow_pickle=True)
    body_names = [str(x) for x in z["body_names"].tolist()]
    body_pos = z["body_pos"][frame]   # [B,3]
    body_quat = z["body_quat"][frame]  # [B,4] wxyz
...
def main():
    ...
    ap.add_argument("--npz", required=True, help="reach dir goal_pointflow.npz")
    ...
    ap.add_argument("--title", default="reach + robot + env")
    ap.add_argument("--cube-size", type=float, default=CUBE)
    a = ap.parse_args()
    ...
    print("WROTE", a.out, "| frames", len(data["goal_traj"]), "| robot meshes",
          len(robot_meshes), "| size MB", round(os.path.getsize(a.out) / 1e6, 2))
...
#hud{position:fixed;top:12px;left:12px;z-index:10;background:rgba(22,27,34,.88);padding:12px 16px;border-radius:10px;border:1px solid #30363d;max-width:340px}
...
<p><b style="color:#3fb0ff">蓝 cube</b> 跟随目标轨迹 · 真实机械臂网格(home 位姿) · <b style="color:#c98a3c">16 关键点</b></p>
<p><b style="color:#c678dd">magenta = 基座</b> · <b style="color:#e5534b">红环</b>0.70 · <b style="color:#d29922">琥珀</b>0.60</p>
<p style="color:#8b949e">Drag=orbit, scroll=zoom. 机器人为静态 rest 位姿(仅作空间参照)。</p></div>
<div id=stat>—</div>
...
const K=gt.length, N=po.length, TZ=D.table_top_z;
...
// cube following goal_traj
const cube=new THREE.Mesh(new THREE.BoxGeometry(D.cube_size,D.cube_size,D.cube_size),new THREE.MeshStandardMaterial({color:0x3fb0ff,roughness:0.4,metalness:0.1}));root.add(cube);
// green object pose flow: trail of cube centres along goal_traj
...
// green pose ghosts at each frame (object pose flow)
const ghosts=new THREE.Group();root.add(ghosts);
gt.forEach((t,i)=>{const gm=new THREE.Mesh(new THREE.BoxGeometry(D.cube_size*0.6,D.cube_size*0.6,D.cube_size*0.6),new THREE.MeshBasicMaterial({color:0x3fd37f,transparent:true,opacity:0.15+0.35*(i/Math.max(1,gt.length-1))}));gm.position.set(...w(t.slice(0,3)));gm.quaternion.copy(qxyzw(t.slice(3,7)));ghosts.add(gm);});
// 16 keypoints
const kp=[];const kgeo=new THREE.SphereGeometry(0.008,10,10);
...
function set(fi){
  const t=gt[fi];cube.position.set(...w(t.slice(0,3)));cube.quaternion.copy(qxyzw(t.slice(3,7)));
  const gf=gp[fi];for(let i=0;i<N;i++){kp[i].position.set(...w(gf[i]));kp[i].visible=document.getElementById('kpts').checked;}
  stat.textContent=`frame ${fi}/${K-1} · cube world z=${w(t.slice(0,3))[2].toFixed(3)}`;
...
document.getElementById('flow').onchange=e=>{trail.visible=e.target.checked;ghosts.visible=e.target.checked;};
