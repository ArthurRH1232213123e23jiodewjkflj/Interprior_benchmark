...
`scripts/`. Anyone who prefers Python calls `bench_multi_val.run` directly and
gets the same thing.
"""
...
def cmd_tasks(args: argparse.Namespace) -> int:
    ...
    from bench_multi_val.suites.suite import discover, load_suite
...
def cmd_verify(args: argparse.Namespace) -> int:
    from bench_multi_val.api import DEFAULT_INTERPRIOR_ROOT, DEFAULT_ISAAC_PYTHON
...
def _run(args: argparse.Namespace, policy: str | None) -> int:
    sys.path.insert(0, str(BENCH_ROOT))
    from bench_multi_val.api import (
    ...
    report = run(
        ...
        # Pass None through when the flag is absent so api.run() can apply its
        # precedence: explicit flag > the suite's own interprior_root > candidate
        # scan. Substituting DEFAULT_INTERPRIOR_ROOT here made the suite's value
        # unreachable, which pointed this benchmark at ~/pro5000_env -- a checkout
        # with no heterogeneous object-pool fields and a different reset schema.
        interprior_root=args.interprior_root,
