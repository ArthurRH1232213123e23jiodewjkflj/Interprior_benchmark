...
Run (Pro5000 render node, data-production env code):
    ...
    ~/pro5000_env/.venv_isaacsim_pro5000/bin/python -u \\
        ~/benchmark/bench_multi_val/driver.py \\
        --suite ~/benchmark/bench_multi_val/suites/cube_lift_follow_v1.yaml \\
...
def _run(args: argparse.Namespace, root: Path) -> int:
    ...
    from bench_multi_val.adapters.recorded_teacher import RecordedTeacherAdapter
    from bench_multi_val.envs.episode import (
        compose_pose,
    ...
    from bench_multi_val.envs.rotation import rotation_matrix_to_quat_wxyz  # noqa: F401
    from bench_multi_val.physics import verify as V
    from bench_multi_val.render.rollout_viewer import (
        CENTER_PATH_COLORS,
        CENTER_PATH_WIDTHS,
        build_center_path_guide,
        write_rollout_html,
    )
    from bench_multi_val.suites.suite import load_suite
    ...
    # per benchmark package (bench_multi_val, bench_multi_val, ...) and the only
    ...
    if inproc_mode:
        ...
        from bench_multi_val.adapters.flow_policy_v10 import FlowPolicyV10Adapter
        ...
        from bench_multi_val.adapters.subprocess_adapter import SubprocessAdapter
    ...
    # IsaacLab's default log dir is RELATIVE, and main() chdir'd into
    # interprior_root so the task yaml's relative asset paths resolve. That put
    # IsaacLab's makedirs inside the checkout, which is fine when the checkout is
    # ours and fatal when it belongs to someone else -- this benchmark reads env
    # code from another user's read-only tree, so it died in
    # SimulationContext.__init__ with PermissionError on <root>/outputs before
    # the scene existed. Point it at our own run directory instead, the way the
    # upstream eval does (physx_rollout.py:950).
    cfg.sim.log_dir = str(args.out / "isaaclab_logs")
    ...
    # ------------------------------------------------- per-case object assets
    # Multi-object: each case has its own object, so the asset cannot come from
    # the profile (which is one file for every case). It is joined per case from
    # object_catalog.parquet and pinned into the pool fields HERE -- after the
    # profile overlay above, which would otherwise clobber them, and before the
    # env is built, which is the last moment the asset can be chosen.
    #
    # The gate is verify.verify_case_asset_pin / align_to_pool_order rather than
    # the profile, because the pin's hasattr fallback (upstream's, deliberately
    # kept) means a checkout without pool fields pins NOTHING while every call
    # returns cleanly -- 436 cases against one object, physics gate green.
    asset_metadata: list[dict] | None = None
    pool_info = None
    case_object_uids = [getattr(c, "object_uid", None) for c in cases]
    if any(case_object_uids):
        missing_uid = [i for i, uid in enumerate(case_object_uids) if not uid]
        if missing_uid:
            raise SystemExit(
                f"cases {missing_uid} lack object_uid while others have it; a "
                "multi-object suite must name an object for every case"
            )
        from bench_multi_val.envs.objaverse_assets import (
            align_to_pool_order,
            build_object_pool,
            resolve_case_asset,
        )
        ...
        asset_metadata = [
            resolve_case_asset(case.shard, uid, verify_sha256=True)
            for case, uid in zip(cases, case_object_uids)
        ]
        pool_info = build_object_pool(cfg, asset_metadata)
        print(f"[replay] object pool: {pool_info['pool_size']} assets, "
              f"{len(set(pool_info['urdfs']))} distinct "
              f"(catalog={Path(asset_metadata[0]['catalog_path']).name})", flush=True)
        problems = V.verify_pool_pin(cfg, asset_metadata)
        if problems:
            raise RuntimeError(
                "object pool pin failed:\n" + "\n".join(f"  {p}" for p in problems)
            )
        print("[replay] object pool pin verified", flush=True)
    ...
    if asset_metadata is not None:
        # MultiUsdFileCfg assigns USD paths in SOURCE-PRIM order, which is not
        # numeric env order (env_10 sorts before env_2). So env i need not hold
        # pool entry i, and every list indexed by case must be permuted to match
        # the envs before any plan or state is sent. Upstream does exactly this
        # (render_objflownet_v10_rgbd_worker.py:363-374).
        order = align_to_pool_order(inner, n)
        if order != list(range(n)):
            print(f"[replay] env/pool order differs from case order: {order}; "
                  "permuting cases to follow the envs", flush=True)
            cases = [cases[i] for i in order]
            episodes = [episodes[i] for i in order]
            asset_metadata = [asset_metadata[i] for i in order]
            if not policy_mode:
                adapters = [adapters[i] for i in order]
            contracts = [contracts[i] for i in order]
        else:
            print("[replay] env/pool order matches case order", flush=True)
...
def _v10_metrics(
    ...
    from bench_multi_val.envs.episode import points_from_pose
