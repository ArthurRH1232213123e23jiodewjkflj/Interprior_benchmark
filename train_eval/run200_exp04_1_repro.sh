...
OUT=${OUT:-$HOME/bench_runs/exp04_1_repro_test200_0830}
GPUS=(1 2 3 4 5 7)
TOTAL=200
HTML_CASES=5          # requested: 5 render pages
...
for i in "${!GPUS[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$gpu nohup $ISAAC -u scripts/bench_replay.py \
        ...
        --policy-cmd "$PY $BENCH/train_eval/policy_server_exp04.py --ckpt $BENCH/train_eval/ckpt/exp04_1_repro_ckpt_0036000.pt --model-config $BENCH/train_eval/ckpt/exp04_1_repro_model_config.yaml --strict-finite --fixed-replan-schedule --replan-interval 30" \
